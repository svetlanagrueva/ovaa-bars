...
// Import after mocks
import { GET } from "@/app/api/speedy/offices/route"
import { NextRequest } from "next/server"

function makeRequest(url = "http://localhost:3000/api/speedy/offices", ip = "1.2.3.4") {
  return new NextRequest(url, {
    headers: { "x-forwarded-for": ip },
  })
}

describe("GET /api/speedy/offices", () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it("returns slim office payload", async () => {
    mockGetOffices.mockResolvedValueOnce([
      {
        id: 1,
        name: "Офис Дружба",
        address: { siteName: "София", fullAddressString: "бул. Цариградско 115" },
      },
      {
        id: 2,
        name: "Офис Център",
        address: { siteName: "Пловдив", fullAddressString: "ул. Княз Борис 20" },
      },
    ])
    ...
    expect(data.offices[0]).not.toHaveProperty("type")
  ...
  it("passes city query param to getOffices", async () => {
    ...
    await GET(makeRequest("http://localhost:3000/api/speedy/offices?city=София"))
  ...
  it("returns 502 when Speedy API fails", async () => {
    mockGetOffices.mockRejectedValueOnce(new Error("Speedy API error: 500"))
  ...
  it("rate limits after 30 requests from same IP", async () => {
    ...
    const ip = "speedy-rate-limit-test-ip"
  ...
  it("allows requests from different IPs independently", async () => {
    ...
    for (let i = 0; i < 30; i++) {
      await GET(makeRequest(undefined, "speedy-ip-a"))
    }

    // IP-B should still work
    const res = await GET(makeRequest(undefined, "speedy-ip-b"))
    expect(res.status).toBe(200)
  })
})
