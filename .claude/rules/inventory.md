...
- `order_item_batches` — populated at ship time; records "this order_item consumed N units from this batch". Fully immutable post-insert; `order_item_batches.product_batch_id` SKU must match `order_items.sku`.
...
**At ship time** (admin clicks "Генерирай товарителница"):
- `suggestBatchesForShipment(orderId)` server action returns FEFO (First-Expired-First-Out) defaults: sums `order_items.quantity` per SKU, picks earliest-expiring `active` batches with available quantity, returns `BatchSuggestion[]`.
- Admin reviews/overrides in batch picker dialog (per SKU, per batch quantity inputs).
- Submit calls `confirmShipmentBatches(orderId, allocations)` which validates: order status (must be confirmable for shipment), per-SKU sum equality with order_items.quantity, all selected batches `active` and same SKU as the line, no overcommit vs `batch_quantity_available`. Inserts `order_item_batches` rows in one transaction. Inventory is NOT moved here — `inventory_log` already recorded `order_out` at order creation. The batch table is accounting-of-allocation, decoupled from sellable count.
