...
- Post-shipment outcomes (`delivery_refused`, `package_lost`, `returned`, `recalled`): recorded via `recordOrderOutcome` — a **pure outcome recorder**, single-responsibility. No refund or inventory writes happen inside this action; those are handled separately by `recordRefund` (money, with its own `inventoryAdjustments` for linked restock) and `recordStockMovement` (pure stock). The UI coordinates them as a **guided multi-step flow**: after a successful outcome save, a context-specific "Next step" callout appears with a button that scroll-focuses the existing refund card. For `package_lost` / `returned` / `recalled` the callout recommends immediate refund; for `delivery_refused` it defers ("record refund once the parcel returns"). Each step is its own server action, independently idempotent. This preserves the three-layer separation between audit, money, and goods — `recordOrderOutcome` stays a small, unsurprising event logger.
