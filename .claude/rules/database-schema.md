...
- `id` **text** PRIMARY KEY — 10-char lowercase hex, `DEFAULT lower(encode(gen_random_bytes(5), 'hex'))`, CHECK `^[0-9a-f]{10}$`. **Not a uuid.** Displayed uppercased with a `#` prefix via `formatOrderId()` in `lib/orders.ts` (e.g. `#A1B2C3D4E5`). 16^10 ≈ 1.1T values; the three INSERT paths in `app/actions/stripe.ts` wrap the insert in `insertOrderWithRetry` (3-attempt retry on Postgres `23505`) as a backstop for the ~10⁻⁸ birthday collision. Refunds, withdrawals, invoices, batches, sales, promo codes — and every other id column in the schema — stay `uuid`; only `orders.id` (plus its 8 FK columns) is text. Validation regex: `ORDER_ID_REGEX = /^[0-9a-f]{10}$/i` exported from `lib/orders.ts`; do NOT reuse `UUID_REGEX` on order IDs.
...
- `order_item_batches` — populated by the admin allocation flow before courier-label generation. Records "this order_item consumed N units from this batch". Columns: `id uuid pk`, `order_item_id bigint fk order_items ON DELETE CASCADE`, `product_batch_id uuid fk product_batches ON DELETE RESTRICT`, `quantity integer > 0`, `confirmed_at`, `confirmed_by`, plus override-reason audit columns: `non_fefo_reason text` and `expired_override_reason text` (each nullable; `chk_non_fefo_reason_length` / `chk_expired_override_reason_length` enforce 20–1000 chars when non-null). Unique on (order_item_id, product_batch_id) — multiple batches per item supported via separate rows.
- **Status-conditional immutability** (replaces the old "fully immutable post-insert" rule). Single trigger `trg_order_item_batches_locked` (BEFORE INSERT/UPDATE/DELETE; function `enforce_order_item_batches_locked_after_shipment`) raises only when the parent order's `tracking_number IS NOT NULL`. Lifecycle:
  - `tracking_number IS NULL` → rows are mutable (admin composing the allocation in the "Партиди" card)
  - `tracking_number = '__generating__'` → locked (courier label being generated; prevents an edit racing the label write)
  - real tracking number → locked (shipment issued)
  - `cancelShipment` clears the value back to null → rows become mutable again, audit event `batch_allocation_unlocked_after_shipment_cancelled` records the unlock
...
- `order_audit_events` — append-only unified event log for orders. `(id bigserial, order_id text fk, event_type text, actor text default 'admin', payload jsonb default '{}', created_at timestamptz)`. **Immutable**: `BEFORE UPDATE`/`BEFORE DELETE` triggers reject mutations; correct by appending.
...
- `save_batch_allocation(p_order_id uuid, p_allocations jsonb)` — atomic delete+insert of `order_item_batches` rows with `FOR UPDATE` locks on the parent order and the referenced `product_batches`. Re-validates inside the transaction: order status='confirmed', tracking_number IS NULL, per-SKU sum equality vs `order_items.quantity`, all referenced batches `active`, and `batch_quantity_available` per batch (DELETE before availability check so the order's prior allocation isn't double-counted). The wrapping server action `saveBatchAllocation` does the FEFO-compliance and expired-override gating in TypeScript using `lib/batches/fefo.ts` for friendly errors, then calls this RPC for the safety-net atomic write + audit emission.
- `enforce_order_status_transition()`, `emit_order_audit_events()`, `emit_invoice_audit_events()`, `emit_refund_audit()`, `emit_refund_annotation_audit()`, `enforce_refunds_total()`, `enforce_refunds_append_only_update()`, `set_refunds_updated_at()`, `set_invoices_updated_at()`, `set_withdrawals_updated_at()`, `enforce_withdrawal_status_transition()`, `emit_withdrawal_audit_events()`, `enforce_product_batches_append_mostly_update()`, `enforce_order_item_batches_locked_after_shipment()`, `check_order_item_batch_sku_consistency()`, `enforce_refund_items_quantity_cap()`, `enforce_refund_items_amount_cap()`, `check_refund_item_order_consistency()`, `update_inventory_current()`, `raise_*_immutable*()` — trigger functions, not callable via RPC.
